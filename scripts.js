...
const bgCtx = bgCanvasEl ? bgCanvasEl.getContext("2d") : null;
...
const drawBgFrame = (timestamp = 0) => {
  ...
  const width = Math.max(window.innerWidth, 1);
  const height = Math.max(window.innerHeight, 1);
  const base = BG_COLORS[0];
  const t = timestamp * 0.001; // acelera el movimiento
  ...
  // Círculos suaves y animados
  for (let i = 1; i < BG_COLORS.length; i += 1) {
    const [r, g, b] = BG_COLORS[i];
    const speed = 0.12 + i * 0.07;
    const phase = t * speed + i * 1.8;
    const wobble = Math.sin(t * 0.25 + i) * 0.08;
    const posX = width * (0.5 + Math.cos(phase) * (0.18 + wobble));
    const posY = height * (0.5 + Math.sin(phase * 1.08) * (0.18 + wobble));
    const radius = maxSize * (0.62 - i * 0.045);
    const gradient = bgCtx.createRadialGradient(posX, posY, 0, posX, posY, radius);

    gradient.addColorStop(0, `rgba(${r}, ${g}, ${b}, ${0.82 + i * 0.05})`);
    gradient.addColorStop(0.42, `rgba(${r}, ${g}, ${b}, ${0.46 + i * 0.05})`);
    ...
    bgCtx.beginPath();
    bgCtx.arc(posX, posY, radius, 0, Math.PI * 2);
    bgCtx.fill();
  }

  // Destello ligero diagonal
  const sheenWidth = width * 1.1;
  const sheenGrad = bgCtx.createLinearGradient(0, 0, sheenWidth, height);
  sheenGrad.addColorStop(0, "rgba(255, 255, 255, 0.0)");
  sheenGrad.addColorStop(0.48 + Math.sin(t * 0.25) * 0.18, "rgba(255, 255, 255, 0.26)");
  sheenGrad.addColorStop(1, "rgba(255, 255, 255, 0.0)");
  bgCtx.globalCompositeOperation = "soft-light";
  bgCtx.globalAlpha = 0.72;
  bgCtx.fillStyle = sheenGrad;
  bgCtx.fillRect(0, 0, width, height);
